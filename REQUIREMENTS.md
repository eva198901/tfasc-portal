...
> **版本**: v1.0  
> **最後更新**: 2026-04-21（同步文件連結與套件約定；功能敘述仍以原需求為主）  
> **狀態**: 正式需求（實作細節以程式碼與 `tfasc-scraper` API 為準）
...
| **套件管理** | **pnpm**（唯一） | 與 `package.json` / CI 一致；勿使用 npm |
| **狀態管理** | Pinia | 集中管理 API 狀態 |
| **元件庫** | Headless UI + Heroicons | 快速成型，無樣式衝突 |
| **後端 API** | FastAPI（`CrawlingData/tfasc-scraper`） | 正式環境以 Docker 服務為準；`tfasc-portal/server` 僅示範用 |
